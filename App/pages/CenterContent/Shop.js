...
import { shopFxShopInfo,shopFxDetailInfo } from '../../api/shopFx';
...
class Shop extends Component {
  ...
  componentDidMount() {
    InteractionManager.runAfterInteractions(() => {
      this._getShopDetail(this.props.user_id);
      this._getShopGoods(this.props.user_id);
    });
 }

  _getShopDetail(user_id){
    if(user_id){
      this.setState({
        loadingStatus: true
      })
      shopFxShopInfo(user_id).then((shopDetailData)=>{
        this.setState({
          loadingStatus: false
        })
 ...
 _getShopGoods(user_id){
   if(user_id){
     this.setState({
       loadingStatus: true
     })
     shopFxDetailInfo(user_id).then((shopGoodsData)=>{
       this.setState({
         loadingStatus: false
       })
       ...
       if(shopGoodsData.status){
         this.setState({
           goodsStatus:true,
           shopListData:this.state.shopListData.cloneWithRows(shopGoodsData.data),
         })
       }else{
 ...
 _renderPage(
  ...
  render() {
        ...
        console.log(this.state.goodsStatus);
               ...
               <View style={{backgroundColor:'#f5f5f5',flex:1}}>
                 ...
                 <View style={{width:width,padding:12,flexDirection:'column',justifyContent:'space-between',backgroundColor:'#fff'}}>
                   <View style={{height:41,flexDirection:'row',justifyContent:'flex-start'}}>
                     {this.state.shopDetail.data.logo?
                       <Image source={{uri: this.state.shopDetail.data.logo}} style={{height:41,width:41,resizeMode:'stretch',marginRight:17}} />
                       :
                        <Image style={{height:41,width:41,resizeMode:'stretch',marginRight:17}} source={require('../img/shopLogo.png')}></Image>
                     }
                     <View style={{flexDirection:'column',height:41,justifyContent:'space-between'}}>
                       <Text>{this.state.shopDetail.data.shop_name}</Text>
                 ...
                 {/* <View style={{height:40,paddingHorizontal:10,backgroundColor:'#fff',marginBottom:10}}>
                   <Text style={{fontSize:14,color:'#000'}}>店铺详情：
                     <Text style={{color:'#797979'}}>{this.state.shopDetail.data.brief.replace('&lt;p&gt;','').replace('&lt;/p&gt;','')}</Text></Text>
                     <WebView
