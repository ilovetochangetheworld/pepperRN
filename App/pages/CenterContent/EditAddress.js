...
import { performGetAreaAction,performEditAreaAction } from '../../actions/GetAreaAction';
import { performAddAddressAction } from '../../actions/AddAddressAction';
import { performEditAddressAction } from '../../actions/EditAddressAction';
...
componentDidMount(){
  ...
  const {navigator,dispatch,getArea} = this.props;
    this._checkLogin();
    if(this.props.editData.is_default){
      this.setState({
        address:this.props.editData.address,
        name:this.props.editData.consignee,
...
//登录检测
_checkLogin(){
  const {navigator,dispatch,address} = this.props;
    ...
    AsyncStorage.getItem('token').then(
      (result)=>{
        if (result===null){
          ...
          dispatch(performEditAreaAction(1,1,this.props.editData.provice,this.props.editData.city,this.props.editData.county_name));
        }
      }
    )
    .catch((error)=>{
    ...
    AsyncStorage.getItem('token').then(
      (result)=>{
        if (result===null){
          ...
          dispatch(performEditAddressAction(result,this.props.editData.address_id,this.state.name,this.state.phone,'',getArea.selectProvince.area_id,getArea.selectCity.area_id,getArea.selectArea.area_id,this.state.address,this.state.default));
          navigator.pop();
        }
      }
    )
  ...
  if(getArea.Province.status){
    return (
      <View style={{backgroundColor:'#f5f5f5',flex:1}}>
          <CommonHeader title='编辑收货地址' onPress={()=>{this.buttonBackAction()}} />
